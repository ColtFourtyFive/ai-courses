...
- **Tool recommendation:** the module needs to name at least one AI tool the learner can use. The brief says "accounts beyond the AI tool" are acceptable setup. Decision needed: which tool(s) to name? Candidates: Claude.ai (free tier, Anthropic), ChatGPT (free tier, OpenAI), Gemini (free tier, Google). All three work for every recommended build. Module should probably name two to avoid single-tool lock-in and give learners who already use one tool the ability to use what they have. The exact tool names are a vendor-framing decision (per Master-Build-Plan Section 2 locked decisions) — flagging for Ayan as owner.
